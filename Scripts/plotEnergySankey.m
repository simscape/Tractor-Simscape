function plotEnergySankey(logsoutTractorEnergyComputation)
...
%% Implementation
% Compute all possible losses
[inputEnergyTotal,lossFrontMotor,lossRearMotor,lossPump,lossChargePump,lossAux] = computeLosses(logsoutTractorEnergyComputation);
% Energy and labels of all possible losses:
labels  = {'CVT Losses: Front Motor'; 'CVT Losses: Rear Motor'; 'CVT Losses: Pump';...
    'CVT Losses: Charge Pump'; 'CVT Losses: Aux'};
reqEn = round([lossFrontMotor(end); lossRearMotor(end); lossPump(end); lossChargePump(end); lossAux(end)],2);

% Filter out the components that are smaller than 1 J. No need to represent them
reqEn(find(reqEn<0.001))  = [];
labels(find(reqEn<0.001)) = [];

% Draw Sankey graph
plotSankeyVertical([inputEnergyTotal(end)], reqEn, 'kJ', {'From Engine',labels{:},'To Axles'},'Sankey Diagram : Tractor Transmission Energy Computation');

end

%% Subfunction:
function plotSankeyVertical(inputs, losses, unit, labels, title)
...
% title  = 'Power Computation for the Transmission';
%-----------------
...
elseif any(losses < 0) || any(inputs < 0)    
    ...
    inputLabel = sprintf('%s \n %s: %.1f %s', title, labels{1}, inputs(1), unit);
